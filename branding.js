...
(function () {
  ...
  // Circuit-node cursor: a small glowing node with a subtle fading trail.
  if (window.matchMedia && window.matchMedia("(hover: hover) and (pointer: fine)").matches) {
    document.body.classList.add("node-cursor");
    const cc = document.createElement("canvas");
    cc.className = "cursor-canvas";
    ...
    window.addEventListener("mousemove", (e) => {
      ...
      hy = e.clientY;
      present = true;
      trail.push({ x: hx, y: hy, t: performance.now() });
    });
    document.addEventListener("mouseleave", () => (present = false));

    (function cursorFrame(now) {
      while (trail.length && now - trail[0].t > LIFE) trail.shift();
      cx.clearRect(0, 0, cw, ch);
      if (present) {
        // trace-like fading trail
        cx.lineCap = "round";
        for (let i = 1; i < trail.length; i++) {
          const a = trail[i - 1];
          const b = trail[i];
          const age = 1 - (now - b.t) / LIFE; // 1 = fresh
          cx.beginPath();
          cx.moveTo(a.x, a.y);
          cx.lineTo(b.x, b.y);
          cx.strokeStyle = `rgba(168, 85, 247, ${0.05 + 0.3 * age})`;
          cx.lineWidth = 0.5 + 2.4 * age;
          cx.stroke();
          // little circuit "nodes" along the trail
          if (i % 4 === 0) {
            cx.beginPath();
            cx.arc(b.x, b.y, 1.4, 0, Math.PI * 2);
            cx.fillStyle = `rgba(192, 132, 252, ${0.25 * age})`;
            cx.fill();
          }
        }
        // glowing head node
        ...
        cx.beginPath();
        cx.arc(hx, hy, 4.2, 0, Math.PI * 2);
        cx.fillStyle = "#e9d5ff";
        cx.fill();
        cx.shadowBlur = 0;
        cx.beginPath();
        cx.arc(hx, hy, 1.8, 0, Math.PI * 2);
        cx.fillStyle = "#ffffff";
        cx.fill();
      }
